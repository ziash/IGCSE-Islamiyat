...
def get_student_path(name: str):
    return os.path.join(STUDENTS_DIR, name.strip().title().replace(" ", "_"))
...
async def get_syllabus():
    path = os.path.join(DATA_DIR, "syllabus.json")
    data = load_json(path)
    if data: return data
    logger.error(f"Syllabus not found at {path}")
    raise HTTPException(status_code=404, detail="Syllabus file not found")

@app.get("/api/memorize")
async def get_memorize_content():
    path = os.path.join(DATA_DIR, "memorize_content.json")
    data = load_json(path)
    if data: return data
...
@app.get("/api/students/{student_name}/progress")
async def get_student_progress(student_name: str):
    path = os.path.join(get_student_path(student_name), "progress_log.json")
    data = load_json(path)
    return data if data else []

@app.post("/api/exam/generate")
async def generate_exam(setup: ExamSetup):
...
async def evaluate_exam(submission: ExamSubmission):
    ...
    # Save logic
    student_dir = get_student_path(submission.student_name)
...
@app.post("/api/students/{student_name}/memorize-attempt")
async def log_memorize_attempt(student_name: str, attempt: MemorizeAttempt):
    student_dir = get_student_path(student_name)
